...
// insieme. Ogni consegna viene salvata come un unico documento con tutte
// le righe prodotto. I documenti salvati possono essere riaperti e consultati.
...
function dettaglioRicevimento(r) {
  ...
  return `
    <div class="list-card" style="border:1px solid #2b5c3a; margin-top:10px;">
      ...
      ` : '<div style="color:#64748b;">Nessun prodotto registrato.</div>'}
    </div>
  `;
}
...
export async function renderRicezioniPage(container, profilo) {
  ...
  container.innerHTML = `
    ...
    <h3 style="font-size:14px; color:#64748b; margin:16px 0 8px;">
      Ricevimenti del ${dataSelezionata} (${listaGiorno.length})
    </h3>
        ...
        <div class="list-card">
          ${listaGiorno.map((r) => `
            <div class="check-row" style="cursor:default; align-items:flex-start; margin-bottom:8px;">
              ...
              <button type="button" class="btn btn-secondary rc-apri" data-id="${escapeHtml(r.id)}" style="flex:0 0 auto; white-space:nowrap;">
                Apri
              </button>
            </div>
  ...
  container.querySelectorAll('.rc-apri').forEach((btn) => {
    btn.addEventListener('click', () => {
      ricevimentoAperto = btn.dataset.id;
      renderRicezioniPage(container, profilo).then(() => {
        const dettaglio = container.querySelector('#rc-chiudi-dettaglio');
        if (dettaglio) {
    ...
    wrap.innerHTML = righeCorrenti.map((riga, i) => `
      ...
      >
        <div style="min-width:0;">
          ...
          <textarea rows="2" placeholder="Descrizione completa del prodotto come da DDT" data-campo="nome" style="width:100%; min-height:58px; resize:vertical; box-sizing:border-box; white-space:pre-wrap; overflow-wrap:anywhere;">${escapeHtml(riga.nome)}</textarea>
        </div>
    ...
    fileInput.addEventListener('change', (e) => {
      ...
      reader.onload = async () => {
        try {
          ...
          mostraStato(result.isDemo
            ? '⚠️ Modalità demo: attiva GEMINI_API_KEY su Vercel per la lettura reale.'
            : '✓ Documento letto. Controlla le descrizioni complete prima di registrare il ricevimento.');
        } catch (err) {
          console.error(err);
  ...
  if (salva) salva.addEventListener('click', async (e) => {
    ...
    const voci = righeCorrenti.filter((r) => r.nome.trim()).map((r) => ({
      nome: r.nome.trim(), quantita: r.quantita, lotto: r.lotto, scadenza: r.scadenza, temperatura: r.temperatura
    }));
