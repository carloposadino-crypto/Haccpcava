...
4. Restituisci SOLO un oggetto JSON valido senza blocchi markdown o testo aggiuntivo con la seguente struttura esatta:
{
  ...
  "criticita": "Punti critici di controllo CCP e suggerimenti tecnici"
    ...
    let recipeData;
    try {import { GoogleGenerativeAI } from "@google/generative-ai";

export default async function handler(req, res) {
  // Configurazione header JSON e CORS
  res.setHeader('Content-Type', 'application/json');
  ...
  try {
    ...
    if (!apiKey) {
      return res.status(500).json({ success: false, error: 'GEMINI_API_KEY mancante nelle variabili d ambiente Vercel' });
    }
    ...
    const promptText = `Analizza il contenuto della ricetta ed estrai i dati strutturandoli in JSON rigido secondo queste regole professionali di cucina:
1. Adatta tutte le dosi degli ingredienti esattamente per 20 PORZIONI, dosando spezie/sale con senso professionale.
2. Converti OGNI singola unità di misura esclusivamente in GRAMMI (g).
3. Gli ingredienti devono essere formattati come elenco verticale con sintassi: "Nome Ingrediente: PesoInGrammig" (esempio: "Farina 00: 500g").
4. Restituisci SOLO un oggetto JSON valido senza blocchi markdown con la seguente struttura esatta:
{
  ...
  "criticita": "Punti critici di controllo CCP e suggerimenti tecnici"
    ...
    if (imageBase64) {
      ...
      return res.status(400).json({ success: false, error: 'Nessun dato inviato' });
    }
    ...
    try {
      ...
      return res.status(500).json({ success: false, error: 'L IA non ha restituito un JSON valido: ' + responseText });
    }
